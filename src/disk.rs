...
impl Device {
    ...
    fn device_bit(self) -> u8 {
        match self {
            ...
            Device::PrimarySlave => 1,
            Device::SecondaryMaster => 0,
            Device::SecondarySlave => 1,
        }
    }
}
...
pub fn identify(device: Device) -> Option<u64> {
    unsafe {
        ...
        x86::io::outb(device.port_base_io() + PORT_OFFSET_DRIVE_HEAD, 0xA0 | (device.device_bit() << 4));
...
pub fn read_sector(device: Device, sector_index: u64) -> [u8; SECTOR_SIZE] {
    let sector_index_bytes: [u8; 8] = sector_index.to_le_bytes();
    assert!(sector_index_bytes[6] == 0 && sector_index_bytes[7] == 0);
    unsafe {
        x86::io::outb(device.port_base_io() + PORT_OFFSET_DRIVE_HEAD, 0x40 | (device.device_bit() << 4));
        x86::io::outb(device.port_base_io() + PORT_OFFSET_SECTOR_COUNT, 0);
...
pub fn write_sector(device: Device, sector_index: u64, sector_data: [u8; SECTOR_SIZE]) {
    let sector_index_bytes: [u8; 8] = sector_index.to_le_bytes();
    assert!(sector_index_bytes[6] == 0 && sector_index_bytes[7] == 0);
    let sector_data: [u16; SECTOR_SIZE / 2] = bytemuck::cast_slice(&sector_data).try_into().unwrap();
    unsafe {
        x86::io::outb(device.port_base_io() + PORT_OFFSET_DRIVE_HEAD, 0x40 | (device.device_bit() << 4));
        x86::io::outb(device.port_base_io() + PORT_OFFSET_SECTOR_COUNT, 0);
        x86::io::outb(device.port_base_io() + PORT_OFFSET_SECTOR_NUMBER, sector_index_bytes[3]);
